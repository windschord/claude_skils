...
> このセクションは**機能5（CLAUDE.md同期）**で使用します。レポート出力先: `docs/sdd/reports/claude-md-sync/[YYYY-MM-DD].md`
> 機能2（実装同期チェック）のレポートとは別ファイルとして出力してください。

CLAUDE.mdの仕様記載と実装コードの乖離です。

### 未記載のAPI

| エンドポイント | 実装ファイル | 優先度 |
|---------------|-------------|--------|
| [METHOD /path] | [ファイルパス] | Medium |
...
| テーブル/カラム | CLAUDE.md | 実装 | 優先度 |
|---------------|-----------|------|--------|
| [テーブル.カラム] | [記載内容] | [実装内容] | Medium |

### 不足セクション

CLAUDE.mdに存在すべきだが未作成のセクション:

| セクション | 対応する実装 | 必須/推奨 | 優先度 |
|-----------|-------------|----------|--------|
| API Endpoints | [ルーティングディレクトリ] | 必須 | High |
| DB Schema | [スキーマ定義ファイル] | 必須 | High |
| Services | [サービスディレクトリ] | 必須 | High |
| Environment Variables | [環境変数定義] | 必須 | High |
