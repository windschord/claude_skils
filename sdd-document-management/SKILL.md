...
タスク割り当て:
- メンバーA: 整合性チェック → docs/reports/consistency/ にレポート出力
- メンバーB: 実装同期チェック → docs/reports/sync/ にレポート出力
- メンバーC: アーカイブ対象検出 → docs/reports/archive/ にレポート出力
- メンバーD: ファイル最適化分析 → docs/reports/optimize/ にレポート出力

各メンバーはチェック結果をレポートファイルに出力し、完了をリーダーに報告してください。
修正は実行せず、レポート作成のみ行ってください。
