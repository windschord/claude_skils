...
| 完了済みタスクの蓄積 | アーカイブ |
| ファイルの肥大化 | ファイル最適化 |
| CLAUDE.mdと実装の乖離 | CLAUDE.md同期 |
| アーカイブ後の仕様散逸 | アーカイブ時CLAUDE.md同期 |
...
```text
フルスキャン選択時:
  → Agent tool（isolation: worktree）で4機能を並列実行
  → 各サブエージェントがレポートを作成
...
- **CLAUDE.md同期（必須）**: アーカイブ時に実装結果をCLAUDE.mdのFeature Specification Summaryに反映
...
CLAUDE.mdへの反映対象:
  ...
  - 新規ページ → Pagesセクション
  - 新規WebSocket → WebSocket Endpointsセクション
...
- **不足セクション検出**: CLAUDE.mdに存在すべきセクション（Pages, WebSocket Message Types, State Machines, Business Rules）の有無
