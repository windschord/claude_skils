...
フルスキャンが選択された場合、**必ずAgent tool（`isolation: worktree`）で4機能を並列実行**する。1つのメッセージで4つのAgent toolを同時に呼び出す:
...
**フルスキャン時はAgent tool（`isolation: worktree`）で4機能を並列実行する。**各サブエージェントが独立したworktreeでチェック・レポート作成を行い、メインセッションが結果を統合する。
...
- 1つのメッセージで4つのAgent toolを同時に呼び出して並列実行
