...
- [観点1: セキュリティ](#観点1-セキュリティ)
- [観点2: ドキュメントとの乖離](#観点2-ドキュメントとの乖離)
- [観点3: 可読性・複雑度](#観点3-可読性複雑度)
- [観点4: ライブラリ選定の妥当性](#観点4-ライブラリ選定の妥当性)
- [観点5: PR説明の適切性](#観点5-pr説明の適切性)
- [観点6: 既知脆弱性の検出](#観点6-既知脆弱性の検出)
- [根拠の裏取り（必須）](#根拠の裏取り必須)
- [レビューの進め方](#レビューの進め方)
- [gh CLIでの情報収集コマンド集](#gh-cliでの情報収集コマンド集)
<!-- /TOC -->
...
```web
AWS IAM actions required for [サービス名] site:docs.aws.amazon.com
```

```web
AWS [サービス名] IAM policy actions reference site:docs.aws.amazon.com
```

**GCP: 必要なロール・権限を確認**

```web
GCP [サービス名] IAM roles permissions site:cloud.google.com
```

**Azure: 必要なロール・権限を確認**

```web
Azure [サービス名] RBAC roles permissions site:learn.microsoft.com
```
...
#### 要求との整合性（SDDプロジェクトの場合）

- `docs/requirements/` の要求と実装が一致しているか
- 実装したテストが要求の `verification` に登録されているか
- 設計方針がPR本文の設計ノートに記載され、実装と一致しているか
- データベーススキーマの変更が、対応する要求と設計ノートに反映されているか
...
- ミューテーションスコアが85%以上か（テストが実際にバグを検出できるか）
- ミューテーションスコアが低い場合、テストのアサーションが不十分でないか
...
- **セキュリティ対応姿勢**: セキュリティアドバイザリへの対応速度、過去の脆弱性対応履歴（※具体的なCVEチェックは[観点6: 既知脆弱性の検出](#観点6-既知脆弱性の検出)で実施）
...
# yarn プロジェクトの場合（v1.x: Classic）
yarn audit --json
# yarn v2.x以降（Modern / Berry）の場合
# yarn npm audit --json
