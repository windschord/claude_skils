...
| `fix_strategy.approach` | 推奨する修正アプローチ。修正の意図・方針を具体的に記載（例: 「既存のバリデーションユーティリティを使用してサニタイズ処理を追加する」） |
| `fix_strategy.alternatives` | 代替の修正案。トレードオフを添えて記載。代替案がない場合は空配列 |
| `fix_strategy.impact` | 修正が波及する範囲（関連ファイル、依存コンポーネント、API互換性等）。影響がない場合は「なし」 |
| `fix_strategy.effort` | 修正の規模感。`small`（1ファイル内の軽微な変更）/ `medium`（複数ファイル変更）/ `large`（設計変更を伴う） |
...
```json
{
  ...
  "fix_strategy": {
    ...
    "alternatives": ["ORMのクエリビルダーを使用する方法もあるが、現在のプロジェクトでは生SQLを使用する方針"],
...
- **AI向け（detailsブロック）**: `fix.old_text`/`fix.new_text`で機械的に修正を適用可能。`fix_strategy`で修正の方針・代替案・影響範囲を提示。`context`で正確な位置を特定可能。`verification_hint`で修正後のチェックポイントを提示
