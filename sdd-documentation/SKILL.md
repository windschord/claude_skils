...
逆順レビュー時も同様: 3層（requirements/design/tasks）すべて存在する場合はチームで並列レビュー。
...
| サブスキル | チーム利用条件 |
|-----------|--------------|
| **task-executing** | 並列実行可能タスク3つ以上 → チーム作成 |
| **sdd-troubleshooting** | 原因候補3つ以上 → チームで並列調査 |
| **sdd-document-management** | フルスキャン時 → チームで4機能並列 |
| **逆順レビュー** | 3層すべて存在 → チームで並列レビュー |

### チーム運用ルール

1. **リーダーは実装禁止**: リーダーは自ら実装・コード変更を行わない。タスク分配、進捗監視、メンバーの成果レビュー、index.md/TodoWriteの一括更新に専念する
