...
## 並列処理活用ワークフロー（Agent tool + isolation: worktree）
...
フェーズ4: タスク計画     → 単一セッション
                           ★ TodoWriteにタスク一覧を同期 ★
フェーズ5: 逆順レビュー   → Agent tool（読み取り専用、並列レビュー）
フェーズ6: タスク実行     → Agent tool + worktree（並列実行）
                           ★ 完了ごとにTodoWriteを更新 ★
フェーズ7: 完了           → 単一セッション
```
...
1. docs/sdd/tasks/index.mdから並列実行可能なタスクグループを確認
2. 1つのメッセージで複数のAgent tool（`isolation: worktree`）を同時に呼び出す
3. 各サブエージェントが独立したworktreeでTDD手順に従い実装
4. 全サブエージェント完了後にworktreeの変更をマージ
5. 統合テストを実行
6. TodoWriteを一括更新
7. 逆順レビューを実施
...
1. 各仮説にAgent tool（読み取り専用）を割り当て
2. 1つのメッセージで複数のAgent toolを同時に呼び出し並列調査
3. メインセッションで調査結果を統合
4. 最有力の根本原因を特定
...
**注意**: レポート作成は並列（worktreeで隔離）、修正実行は順次

## タスク同期（TodoWrite連携）

### 概要

docs/sdd/tasks/のタスクとClaude CodeのTodoWriteを同期し、ユーザーに進捗をリアルタイム表示します。
