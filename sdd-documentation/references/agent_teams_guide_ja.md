...
このガイドでは、SDDワークフローにおける並列処理の活用方法を説明します。**Agent tool**（`isolation: worktree`オプション付き）を主要な並列実行手段として使用し、独立したタスクを複数のサブエージェントが並列で処理することで開発効率を向上させます。
...
- **常に利用可能**（実験的機能フラグ不要）
- `isolation: worktree` で各サブエージェントが独立したgit worktreeで動作し、**ファイル競合を自動回避**
...
│  Q3. メンバー間の議論・相互反証が必要？               │
│      YES → エージェントチーム                         │
│      NO  → Agent tool（isolation: worktree）         │
│                                                     │
│  補足:                                               │
│  ・2タスクでもAgent toolなら低コストで並列化可能      │
│  ・3タスク以上は積極的に並列化すること                │
│  ・ファイル競合はworktreeが自動解決するため            │
│    Q2のみ確認すればよい                               │
│                                                     │
└─────────────────────────────────────────────────────┘
```
...
| **ファイル競合** | worktreeで自動回避 | 手動でファイル分離が必要 |
...
各サブエージェントがgit worktree（独立した作業ツリー）で動作するため、同じファイルを編集しても競合しない。
