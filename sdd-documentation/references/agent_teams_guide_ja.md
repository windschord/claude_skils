...
プラン承認は原則不要です。ただしDB操作・認証設定変更等の高リスクタスクでは承認を要求してください。
...
共通ルール:
- Jules: 依頼文にPR作成先を明記し、julesコマンドを連続投入して非同期で並行依頼
...
※ セッション中断・再開時: 前回セッションのJules番号を引き継ぎ、続番から
  開始する。番号リセットは行わない（例: 前回Jules-5まで使用→再開時はJules-6から）。
```
