...
ヒアリングからIPA非機能要求グレード2018へのマッピング、顧客レビュー用Excel出力、指摘取込、運用設計書（Word）生成までを一貫して行うスキルです（旧operations-design・旧ipa-nfr-operations-designの後継）。
...
- 全88項目のヒアリング質問を項目マスタで定義（優先度 高/中/低 の事前分類つき）
- ヒアリング結果の管理（正データは人間可読なYAML1ファイル。実行時にインメモリSQLiteでSQL制約検証）
- 顧客レビュー用Excelの出力（優先度3段階の色分け・顧客判定プルダウン・グレード外要求の記入欄）
- 顧客指摘の取り込みと承認フローつきの対応方針管理
- 整合性の機械チェック（重複項目の値一致・バックアップ間隔とRPOの対応 等）
- 運用設計書のテンプレート機械生成とWord変換（全項目カバレッジ検証で記載漏れを排除）

**役割分担:**

AIはヒアリングと対応方針の判断のみを担い、ドキュメント生成・整合性チェックはスクリプトが機械的に実行します（コスト削減と矛盾・記載漏れの排除）。
