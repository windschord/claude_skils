...
梅澤伸嘉氏の「キーニーズ法」に基づき、生活シーン・場面ごとにQ1（生活ニーズの把握）→Q2（現状行動の深掘り）→But（問題点・不満の抽出）の3段階で潜在ニーズを引き出し、会話ログの保存と「強さ×未充足度」の2軸分析までを一連で行うスキルです。
...
- 場面ごとのCAS的深掘り質問（Q1→Q2→But）
- 話題転換・視点変更による視野拡大
- ユーザーが回答に困った際の選択肢提示（例外的な足場作り）
- 質問時にこちらの予想・想定を先に述べない運用の厳守（ユーザーの思考への誘導防止）
- 会話ログのターン単位保存と「強さ×未充足度」マトリクスによる分析レポート作成
...
### project-phase-risk-audit

大規模システムリプレース・移行プロジェクトについて、マルチベンダー管理とリスクマネジメントに長けたPMOの視点で、見落とされがちな潜在リスクを洗い出すスキルです。正常系の設計や特定ベンダーの担当範囲内には現れにくい「ベンダー間の隙間」「業務影響」「異常系」の死角を、フェーズ別の4つの分析フレームワーク（各4視点、カスタム指定も可）で特定します。
...
| フレームワーク | 想定フェーズ | 4視点 |
|---------------|-------------|-------|
| 汎用 | 総合テスト終盤・移行リハーサル前 | データライフサイクルと異常系／周辺アクター／時間軸の境界／マルチベンダー責任分界点 |
| 要件定義フェーズ特化 | 要件定義中 | 現行仕様のブラックボックス化と非互換性／非機能要件／データ移行要件／セキュリティと監査要件 |
| 運用設計フェーズ特化 | 運用設計中 | 障害時の責任分界点／日常運用のプロセスと権限／計画停止とパッチ適用／BCPとバックアップ復旧 |
| 負荷試験フェーズ特化 | 負荷試験の計画段階・実施直前 | シナリオの現実性と突発的スパイク／マルチベンダー連携と境界ボトルネック／本番同等のデータ状態とキャッシュ／限界突破時の挙動と縮退運転 |
