...
| 条件 | 判定 |
|------|------|
| 並列実行可能なタスクが3つ以上ある | YES/NO |
| 各タスクが異なるファイルセットを対象 | YES/NO |
| タスク間に依存関係がない（同一グループ内） | YES/NO |
...
2. エージェントチームを作成（デリゲートモード）
   ↓
3. 各チームメンバーにタスクを割り当て
   - スポーンプロンプトにタスクファイルパス・対象ファイル・技術コンテキストを含める
   ↓
4. チームメンバーが独立して実装
   ↓
5. リーダーが全メンバーの完了を待機
   ↓
6. リーダーがdocs/tasks/index.mdを一括更新
   ↓
7. リーダーがTodoWriteを一括更新
   ↓
8. 次の依存グループへ（条件を満たせば再びチーム作成）
   ↓
9. 全タスク完了後に逆順レビュー
   ↓
10. チームをクリーンアップ
...
- docs/tasks/index.mdは編集しない（リーダーが一括更新）
...
| 基準 | エージェントチーム | サブエージェント |
|------|-------------------|-----------------|
| 3つ以上の独立タスク | YES → チーム優先 | 可能だがチーム推奨 |
| 1-2個のタスク | オーバーヘッド大 | YES → サブエージェント |
| メンバー間の問題共有が必要 | YES → チーム | 不可能 |
| 結果のみ必要 | オーバーヘッド大 | YES → サブエージェント |
