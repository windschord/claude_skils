...
4. チームメンバーが独立して実装
   ↓
5. リーダーは実装せず、各メンバーの進捗確認と成果レビューに専念
   ↓
6. 全メンバー完了後、リーダーがdocs/tasks/index.mdとTodoWriteを一括更新
   ↓
...
リーダーは実装コードや実装ファイルの編集を**一切行わない**。以下に専念する:
- タスク分配とスポーンプロンプトの作成
- 各メンバーの進捗監視
- メンバーの成果（コード・テスト結果）のレビュー
- docs/tasks/index.mdとTodoWriteの一括更新（管理系更新はリーダーが行う）
- 逆順レビューの実施とチームクリーンアップ
