...
flowchart TD
    ...
    D -.- D1["全体構造を説明<br/>必須/推奨/オプションを明示"]

    subgraph LOOP["3. セクションごとの反復（for each section）"]
        S1["3.1. ヒアリング<br/>AskUserQuestionで質問"] --> S2["3.2. セクション作成<br/>ヒアリング情報のみ使用"]
        S2 --> S3["3.3. ユーザー確認・承認"]
        S3 --> S4["3.4. 次のセクションへ"]
    end

    F -.- F1["全セクション間の矛盾を検出<br/>AI自身が自動修正"]
    G -.- G1["自動修正後の設計書を提示<br/>ユーザーの最終確認"]
```
...
```mermaid
flowchart TD
    subgraph LOOP["for each section in 推奨セクションリスト"]
        S1["3.1. このセクションに必要な情報をヒアリング"] --> S2["3.2. このセクションのみ作成"]
        S2 --> S3["3.3. ユーザーに確認・承認"]
        S3 --> S4["3.4. 次のセクションへ"]
    end
```
