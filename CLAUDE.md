...
1. **リーダーは実装禁止**: リーダーは実装コードや実装ファイルの編集を行わず、進捗監視・成果レビュー・結果統合に専念する（docs/tasks/index.mdやTodoWrite等の管理系更新はリーダーが行う）
...
SDDスキルの実行時に長時間無応答になる問題を防止するためのルール。

### 1フェーズ1応答の原則

- **1つのサブスキル工程が完了したら、必ずユーザーに結果を報告して応答を返す**
- 複数のサブスキルを連続実行しない（requirements→design→tasksを一気に実行しない）
- 各フェーズの所要時間目安は3-10分。10分を超える場合はスコープを見直す

### テンプレート・リファレンスの遅延読み込み

- テンプレートやリファレンスは**使用する直前に1つずつ**読み込む
- 事前に全ファイルを一括読み込みしない（コンテキスト肥大化によるタイムアウトの原因）

### エージェントチーム判定の適切なタイミング

- ドキュメント作成フェーズ（requirements/design/tasks）ではチーム判定不要
- タスク実行フェーズとトラブルシューティングでのみ判定を実施

### 進捗の可視化

各サブスキル実行中は処理ステップをユーザーに出力して進捗を可視化すること:
...
長文の入力を受け取った場合、一括分類を避け段階的に処理すること:

1. **Phase 1**: 必須情報のみ確認 → 不明点があれば質問
2. **Phase 2**: 機能要件の整理
3. **Phase 3**: 非機能要件の整理
