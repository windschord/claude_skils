...
- praise: 本文に肯定語（`LGTM`, `Great`, `Nice`, `Well done`等）を含む場合は候補とする
  - ただし修正指示語（`fix`, `change`, `update`, `remove`, `should`, `must`, `please`等）を含む場合はスキップしない
  - 肯定語リストと修正指示語リストは外部設定として管理し、誤分類を防ぐ
