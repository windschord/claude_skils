...
- `path`と`line`が明示的
- コード修正の提案を含むことが多い
- suggestion形式（``suggestion``コードブロック）を使用する場合がある
...
suggestion形式のコメントは、API応答の`start_line`・`line`・`start_side`・`side`で特定した置換対象レンジを提案コードで置き換えて対応する。単一行前提で処理せず、複数行suggestion（`start_line`〜`line`）を正確に扱うこと。適用前に`start_side`/`side`がdiffの左右コンテキスト（LEFT/RIGHT）と一致することを検証し、diffコンテキストの整合性を確認した上で適用する。不一致が検出された場合はauto-fixを中止しmanual-requiredにフォールバックする。

#### 複数行suggestionの処理手順

1. APIレスポンスから`start_line`と`line`を取得し、置換対象の行範囲を特定する
   - `start_line`が存在しない場合は単一行（`line`のみ）として扱う
   - `start_line`が存在する場合は`start_line`〜`line`の範囲を置換対象とする
2. `start_side`/`side`でdiffのコンテキスト（LEFT=変更前、RIGHT=変更後）を確認する
3. 対象ファイルの該当行範囲を``suggestion``コードブロック内のコードで置換する
4. 適用前のバリデーション:
   - 対象行範囲がファイルの行数を超えていないこと
   - `start_line` <= `line`であること
   - diffコンテキストの整合性（side値の一致）
...
- praise: 以下の2条件を**両方**満たす場合にpraiseと判定する
  - 条件1: 本文に肯定語（`LGTM`, `Great`, `Nice`, `Well done`等）を含む
  - 条件2: 本文に修正指示語（`fix`, `change`, `should`, `must`, `please`, `need`等）を含まない
  - 肯定語リストと修正指示語リストは外部設定（辞書）として管理し、プロジェクトごとの誤分類を防ぐ
- summary: CodeRabbitの`## Summary`, `## Walkthrough`セクション。HTMLコメント `<!-- This is an auto-generated comment by CodeRabbit -->` を含む投稿もSummaryとして判定する
