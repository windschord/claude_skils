...
```text
1. jules-api/scripts/list-sources.sh でリポジトリのソース名を取得
...
1. セッション作成が失敗した場合:
   - 【重要】タイムアウト等の曖昧な失敗時、ユーザーの作業承認を待たずに再実行してはならない。
     ...
     （jules-api/scripts/create-session.sh は同名タイトルの既存セッションを検知して
     自動で作成を中断するが、生のcurlを使う場合は list-sessions.sh で重複がないか
     必ず確認してから再実行する）
