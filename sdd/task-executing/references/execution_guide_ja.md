...
- 技術的文脈
- 参照すべき既存コード
- 対象要求（`docs/requirements/` の該当する要求とユーザーストーリー）
- PR本文に記載済みの設計ノート（あれば）
...
実装→タスク→要求の整合性を確認。設計はPR本文の設計ノートと突き合わせる。
...
【Agent tool 呼び出し1】
  ...
  prompt: |
    ...
    ## 参照ドキュメント
    要求: REQ-0001, REQ-0002（docs/requirements/generated/traceability.md）
    設計: PR本文の設計ノート（未作成なら本タスクで検討してPRに記載する）
...
設計書は存在しないため、PR本文に書いた設計ノートと実装を突き合わせる。

| チェック項目 | 確認内容 |
|-------------|---------|
| 要求カバレッジ | 実装が対応する要求（REQ-XXXX）を満たしているか |
| 検証手段の登録 | 実装したテストが要求の `verification` に登録されているか |
| 検査の通過 | `reqctl.py validate --strict --check-tests` がエラー・警告ゼロで終了するか |
| 設計ノートとの一致 | PR本文に書いた方式（データ構造・API形状・処理フロー）と実装が一致するか |
| 過剰実装チェック | 要求にない機能が実装されていないか |

#### 3. 要求側に問題が見つかった場合

実装してみて要求の矛盾・実現不能が判明した場合、**実装で辻褄を合わせない**。

| 状況 | 対応 |
|------|------|
| 要求どうしが矛盾している | 実装を止め、`reqctl.py impact <ID>` で影響範囲を出し、変更提案レポートを作成してユーザー承認を得る |
| 要求に書かれていない挙動が必要 | 先に要求を追加する（`requirement-management` の追加フロー） |
| 要求が実現不能 | 何が不能かを具体的に示し、要求の変更提案を出す |

詳細は `requirement-management/references/design_in_pr_ja.md` の「設計中に要求の問題が見つかった場合」を参照。
...
1. Task 1.1の受入基準「エラーハンドリングが適切に実装されている」
   が満たされていません。
   → 400エラー時のレスポンス形式がPR本文の設計ノートと異なります。

【タスク → 要求の不整合】
2. REQ-0003「パスワードリセット機能」に対応する実装がありません。
3. REQ-0005 の verification にテストが登録されておらず、
   reqctl.py validate --check-tests が E-NOVERIFY を報告しています。
