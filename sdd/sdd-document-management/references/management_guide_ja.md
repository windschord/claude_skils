...
- [CLAUDE.md同期 詳細ガイド](claude_md_sync_ja.md) - CLAUDE.mdと実装コードの乖離検出・更新の詳細手順
...
> **注意**: ここでの「CLAUDE.md」は、このスキルリポジトリのCLAUDE.mdではなく、**SDDワークフローの対象プロジェクトのCLAUDE.md**を指します。対象プロジェクトのCLAUDE.mdを「生きた仕様書」として維持することがこのスキルの目的です。

情報の有用性は以下の階層で整理されます:

| レベル | ドキュメント | 役割 | 参照タイミング |
|--------|-------------|------|---------------|
| **1（最重要）** | 対象プロジェクトのCLAUDE.md | 生きた仕様書 | 常にコンテキストに入る |
| **2** | コード自体 | 真実の源泉 | 必要時に読む |
| **3** | アクティブSDD | 計画書 | 実装時にのみ参照 |
| **4** | アーカイブSDD | 過去の記録 | ほぼ不要 |

**核心原則**:
- **CLAUDE.md** = 「今動いているもの」の仕様書（現在形）
- **アクティブSDD** = 「これから作るもの」の計画書（未来形）
- **アーカイブSDD** = 「過去の計画」の考古学的資料（過去形）

完了したプロジェクトのSDDをarchiveに移動するだけだと、実装した仕様がどこにも「現在形」で残らない。そのため:

```text
SDD完了 = archive移動 + CLAUDE.md更新（セット）
```

このステップがドキュメント管理において最も重要なルールです。
...
> **Issueモード（デフォルト）**: DONEタスク＝closeされたIssue（`label:sdd:task state:closed`）。close自体がアーカイブに相当するため、以下のファイル移動（手順5-6）は不要。手順1の検出は `mcp__github__list_issues`（state:closed, labels:["sdd:task"]）で行い、手順3のCLAUDE.md反映はIssueモードでも必須。以下はファイルモード（オプション）の手順。
...
3. CLAUDE.md反映対象の抽出
   - アーカイブ対象のSDD（requirements/*.md, design/*.md, tasks/*.md）から新規API、DB、サービス等を抽出
   - CLAUDE.mdの既存記載と比較し、未記載の項目をリストアップ

4. ユーザー承認
   - 対象リストを提示（アーカイブ対象 + CLAUDE.md更新対象）
   - 依存関係がある場合は警告
