...
> **タスク管理先の前提**: SDDタスクはデフォルトで **GitHub Issue**（`label:sdd:task`）で管理される。ファイルモード（オプション）時のみ `docs/sdd/tasks/` にファイルが存在する。各機能はモードに応じて参照先を切り替えること。
> - **整合性/実装同期チェック**: タスク一覧は `mcp__github__list_issues`（labels: `sdd:task`）で取得（ファイルモードは docs/sdd/tasks/）
> - **アーカイブ**: Issueモードでは「DONE＝closed Issue」で表現されるため、closeによりアーカイブ相当が完結する（`docs/sdd/archive/tasks/` への移動はファイルモードのみ）。完了プロジェクトの仕様をCLAUDE.mdへ転記するステップはIssueモードでも必須。
> - 本SKILL・テンプレート中の `docs/sdd/tasks/...` の記載はファイルモードの例。Issueモードでは対応するIssue操作に読み替える。
