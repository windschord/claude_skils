...
- [ワークフロー詳細](#ワークフロー詳細)
  - [フェーズ1: 初期化](#フェーズ1-初期化)
  - [フェーズ2: 要件定義](#フェーズ2-要件定義)
  - [フェーズ3: 設計](#フェーズ3-設計)
  - [フェーズ4: タスク計画](#フェーズ4-タスク計画)
  - [フェーズ5: 逆順レビュー](#フェーズ5-逆順レビュー)
  - [フェーズ6: 完了](#フェーズ6-完了)
- [使い分けガイド](#使い分けガイド)
  - [一括フロー（推奨）](#一括フロー推奨)
  - [個別フロー](#個別フロー)
- [トラブルシューティング](#トラブルシューティング)
  - [不整合が多い場合](#不整合が多い場合)
  - [情報が不足している場合](#情報が不足している場合)
  - [スコープが大きすぎる場合](#スコープが大きすぎる場合)
- [並列処理活用ワークフロー（Agent tool + isolation: worktree）](#並列処理活用ワークフローagent-tool--isolation-worktree)
  - [概要](#概要-1)
  - [並列処理活用タイミング](#並列処理活用タイミング)
  - [タスク実行フェーズの並列処理](#タスク実行フェーズの並列処理)
  - [トラブルシューティングでの並列処理](#トラブルシューティングでの並列処理)
  - [ドキュメント管理での並列処理](#ドキュメント管理での並列処理)
- [タスク同期（TodoWrite連携）](#タスク同期todowrite連携)
  - [概要](#概要-2)
  - [同期タイミング](#同期タイミング)
  - [ステータス対応](#ステータス対応)
- [ベストプラクティス](#ベストプラクティス)
...
2. 不明点をユーザーに確認
3. `mcp__github__get_me` で対象リポジトリ・権限を確認、ラベル（`sdd:task`等）を準備
4. 各タスクを `task-planning/assets/templates/task_issue_template_ja.md` に従い本文構築
5. `mcp__github__issue_write`（create）で各タスクをIssue化（タイトル `[TASK-XXX]`、ラベル付与）
6. 各Issue本文にTDD手順・受入基準・依存Issue番号を記載
7. 依存関係・並列グループを整理（`sdd:group-*`）
8. ユーザーに起票Issue番号一覧を提示・承認

> ファイルモード（オプション）は従来通り `tasks/index.md` + `phase-N/TASK-XXX.md` を作成する。

**チェックポイント**:
- [ ] タスクが適切な粒度である（20-40分）
- [ ] 各タスクに受入基準がある
- [ ] 推測に基づく実装指示がない
- [ ] 全タスクが起票されている（Issueモード: `label:sdd:task` で確認 / ファイルモード: index.mdにリンク）
- [ ] ユーザーの承認を得た
...
1. 並列実行可能なタスクグループを確認（Issueモード: `label:sdd:task` ＋ `sdd:group-*` / ファイルモード: docs/sdd/tasks/index.md）
2. 1つのメッセージで複数のAgent tool（`isolation: worktree`）を同時に呼び出す
3. 各サブエージェントが独立したworktreeでTDD手順に従い実装
4. 全サブエージェント完了後にworktreeの変更をマージ（コンフリクトがあれば手動解決）
5. 統合テストを実行
6. 完了したタスクごとにTodoWriteを更新
7. 逆順レビューを実施
...
SDDタスク（デフォルト: GitHub Issue `label:sdd:task` / オプション: docs/sdd/tasks/）とClaude CodeのTodoWriteを同期し、ユーザーに進捗をリアルタイム表示します。
