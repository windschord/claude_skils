...
- [パターン5: Jules API + Agent tool ハイブリッド実行](#パターン5-jules-api--agent-tool-ハイブリッド実行)
...
- [サブエージェントのモデル選択ガイド](#サブエージェントのモデル選択ガイド)
  - [認知レベルとモデル推奨](#認知レベルとモデル推奨)
  - [SDDフェーズ別の推奨](#sddフェーズ別の推奨)
  - [使用例](#使用例)
<!-- /TOC -->
...
## パターン5: Jules API + Agent tool ハイブリッド実行

**使用フェーズ**: タスク実行（task-executing）でJules APIが利用可能な場合

**条件**:
- JULES_API_KEYが設定されている
- 3つ以上の独立したタスク
- 開発ブランチが指定されている
...
```text
メインセッション（オーケストレーター）
├── Jules依頼（API経由、非同期PR作成）:
│   ├── Session-1: TASK-001 → 非同期でPR作成
│   ├── Session-2: TASK-002 → 非同期でPR作成
│   └── Session-3: TASK-003 → 非同期でPR作成
...
| Jules APIのみで十分 | ローカル実行不要 | Jules APIスキル単体 |
...
Agent toolでサブエージェントを起動する際、`model`パラメータでモデルを指定できる。タスクの認知レベル（Bloom taxonomy準拠）に応じて適切なモデルを選択する。

### 認知レベルとモデル推奨

| 認知レベル | 説明 | 推奨モデル | 具体例 |
|-----------|------|-----------|--------|
| L1-L2: 記憶・理解 | 情報の検索・要約 | haiku | ファイル一覧取得、設定値確認、既存コードの読み取り |
| L3-L4: 適用・分析 | 既知パターンの実装・コード分析 | sonnet | 定型的なCRUD実装、テスト作成、リファクタリング、コードレビュー |
| L5-L6: 評価・創造 | アーキテクチャ設計・複雑な判断 | opus | 設計判断、複雑なバグ分析、新規アーキテクチャ提案 |

### SDDフェーズ別の推奨

| フェーズ | 認知レベル | 推奨モデル | 理由 |
|---------|-----------|-----------|------|
| 要件定義 | L5-L6 | opus | ユーザー要求の解釈、要件の構造化に高度な判断が必要 |
| 設計 | L5-L6 | opus | アーキテクチャ決定、トレードオフ評価が必要 |
| タスク計画 | L4-L5 | sonnet/opus | 設計からの分解は定型的だが、依存関係分析に判断が必要 |
| タスク実行 | L3-L4 | sonnet | 明確な仕様に基づくコード実装（agents/task-executing.mdで設定済み） |
| 逆順レビュー | L4-L5 | sonnet/opus | 整合性チェックは分析レベル |
| トラブルシューティング | L4-L6 | sonnet/opus | 原因分析の複雑さに応じて選択 |
...
**注意**: `model`パラメータはAgent toolの起動時に指定する。SKILL.mdのフロントマターにはmodelフィールドを追加しない（公式仕様外）。エージェント定義ファイル（`agents/*.md`）のみがフロントマターにmodelフィールドを持てる。
