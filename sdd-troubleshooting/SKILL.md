...
description: エラー・バグ・問題を体系的に分析し修正方針を策定する。テスト失敗、ビルドエラー、実行時エラー、動作不良、バグ報告に対応。「動かない」「おかしい」「期待通りでない」「修正してほしい」「不具合がある」「追加修正が必要」などの問題指摘・修正依頼時に、根本原因を分析してから修正を行う。
version: "1.0.0"
---
...
原因が不明な複雑なバグの場合、エージェントチームを活用して複数の仮説を並列で調査できます。各チームメンバーが異なる仮説を担当し、互いの発見を共有・反証することで、より迅速に根本原因に到達します。
