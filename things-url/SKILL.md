...
Claude Codeで管理しているタスクをThings 3のURLスキームを使って人間のタスク管理アプリと共有するスキルです。docs/sdd/tasks/やTodoWriteのタスクをThings 3に一方向で送信し、人間側でもタスクの閲覧・管理を可能にします。
...
- TodoWriteの現在のタスクリストをThings 3に一方向で送信
...
```bash
# auth-tokenを含むURLは標準出力に表示せず、直接openコマンドで実行する
python3 -c "
...
**セキュリティ上の注意**:
- auth-tokenを含むURLは`print()`で標準出力に表示しないでください。シェル履歴やログにトークンが残るおそれがあります
- 必ず`subprocess.run(['open', url])`で直接実行するか、auth-token部分をマスクして表示してください
