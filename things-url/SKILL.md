...
3. **ステータスのマッピング**（Things → TodoWrite）
   ...
   | Things status | TodoWrite status |
   |---|---|
   | `open` | `pending` or `in_progress`（タグで判定） |
   | `completed` | `completed` |
   | `canceled` | 除外 |
...
「Claude Code」タグや「SDD」タグが付いたタスクのみを取得する場合：
...
tell application "Things3"
    ...
    repeat with t in (allTodos & todayTodos)
...
2. **自動同期は非対応**: Things側での変更のClaude Codeへの反映はユーザーの明示的な指示が必要（自動検知はしない）
