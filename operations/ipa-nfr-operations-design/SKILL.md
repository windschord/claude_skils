...
2. Excelファイルが直接読めない場合:
   - ユーザーに「テキスト/CSV/Markdownに変換して貼り付け」を依頼
   - または「主要な記入済み項目（稼働率、RTO、RPO等）を口頭で教える」を依頼
...
IPA活用シートには複数箇所に同一要件が記載される「重複項目」があります（○マークで示される）。

| 重複ペア | 処理方針 |
|---------|---------|
| A.1.1.1 ＝ C.1.1.1 | 値が一致する場合はそのまま使用。不一致の場合はユーザーに確認（保留） |
| A.1.1.3 ＝ C.2.1.1 | 同上（不一致時はユーザーに確認） |
| A.2.6.1 ＝ C.1.2.7 | 同上（不一致時はユーザーに確認） |
| A.2.6.2 ＝ C.1.2.1 | 同上（不一致時はユーザーに確認） |
| A.4.1.1 ＝ C.3.1.1 | 同上（不一致時はユーザーに確認） |
...
- **Excelシートのパスが指定された場合**: Readツールで読み込みを試みる。読めない場合はユーザーにテキスト変換を依頼
