...
description: ヒアリングからIPA非機能要求グレード2018へのマッピング、SQLite DBでの管理、顧客レビュー用Excel（優先度3段階提示）の出力、顧客指摘の取り込み、運用設計書（Word）の生成までを一貫して行う。非機能要件定義〜運用設計フェーズの運用設計書作成すべてに使用する（旧operations-design・ipa-nfr-operations-designスキルの後継）。Do NOT use for ソフトウェアの機能設計（software-designingを使用すること）。Do NOT use for 業界トレンド調査そのものが目的の場合（WebSearchで直接調査すること）。
metadata:
  version: "1.0.0"
---
...
ヒアリング → IPA非機能要求グレード2018マッピング → DB登録 → 顧客レビュー用Excel出力 → 顧客指摘の取り込み → 運用設計書（Word）生成、までの一連のワークフローを提供します。旧 `operations-design`（ヒアリング手法）と旧 `ipa-nfr-operations-design`（IPAマッピング・運用設計書生成）を置き換える後継スキルであり、顧客提示可能な成果物を作成します。
...
**AIはヒアリングと判断、ドキュメント生成と整合性チェックはスクリプトによる機械処理**という役割分担を厳守する。これによりトークンコストを削減し、DBを単一の情報源としてドキュメント間の矛盾・記載漏れを排除する。

| 作業 | 担当 | 理由 |
|------|------|------|
| ヒアリング・IPAマッピング | AI（AskUserQuestion） | 対話的な引き出しと文脈理解が必要 |
| 顧客指摘への対応方針の判断 | AI＋ユーザー承認 | 業務判断が必要 |
| Excel/Word/設計書Markdownの生成 | スクリプト（機械処理） | DBから決定論的に生成し、矛盾・記載漏れ・転記ミスを排除 |
| 整合性チェック | スクリプト（`check`） | 同一ルールを毎回確実に適用 |

**AIが設計書本文を直接記述・編集してはならない**。値の修正は必ずDB（`register` / `update-feedback`）に対して行い、ドキュメントは再生成する。
...
- DB・Excel・設計書Markdown・Wordの生成と整合性チェックをスクリプト経由で機械的に実行
- 顧客指摘への対応方針をユーザーと合意してからDBに反映（承認フロー）

**このスキルが行わないこと**:
- AIによる設計書本文の直接記述・編集（生成はスクリプトのみ。修正はDB更新→再生成）
...
1. **基本情報の確認**（AskUserQuestionツールを使用）
   ...
   - 確認できたらフェーズ2のDB初期化（init）を先に実行してよい（hearing-sheetを使うため）
2. **質問一覧の機械取得**: `hearing-sheet` で未登録項目の質問・記入例を取得する

```bash
python3 operations/ipa-nfr-workflow/scripts/nfr_workflow.py hearing-sheet --db <path>/nfr.db --priority 高
```
...
python3 operations/ipa-nfr-workflow/scripts/nfr_workflow.py check --db <path>/nfr.db
```
...
2. **運用設計書Markdownの機械生成**（テンプレートへDB値を代入。未登録項目は `[要確認]` が自動挿入され、全88項目のカバレッジが検証される）:

```bash
python3 operations/ipa-nfr-workflow/scripts/nfr_workflow.py generate-design \
  --db <path>/nfr.db --output <path>/design.md
```

   - `accepted` の指摘が要件値の変更を伴う場合は、生成前にregisterでDBへ反映し、反映後に `update-feedback --status reflected` で状態を更新する
...
python3 operations/ipa-nfr-workflow/scripts/nfr_workflow.py export-word \
  --db <path>/nfr.db --design <path>/design.md --output <path>/<システム名>-運用設計書-<YYYYMMDD>.docx
```
...
3. **DBファイルが正**: 選択値・指摘の正データはDB。Excel・設計書Markdown・WordはすべてDBからの機械生成物であり、直接編集して正とすることはしない（顧客の記入は必ずimport-feedbackで取り込み、値の修正はregisterで行って再生成する）
...
### scripts/
- `nfr_workflow.py`: ワークフロー管理CLI。サブコマンド: `init` / `hearing-sheet` / `register` / `status` / `export-excel` / `import-feedback` / `list-feedback` / `update-feedback` / `check` / `generate-design` / `dump` / `export-word`
- `nfr_schema.sql`: DBスキーマの正定義（SQL DDL。initが読み込む）
