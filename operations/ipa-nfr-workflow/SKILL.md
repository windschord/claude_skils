...
### scripts/
- `nfr_workflow.py`: ワークフロー管理CLI。サブコマンド: `init` / `register` / `status` / `export-excel` / `import-feedback` / `list-feedback` / `update-feedback` / `check` / `generate-design` / `dump` / `export-word`
