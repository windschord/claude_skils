...
レビュー開始前に、変更内容とコンテキストから情報を分類する:

1. 変更の説明（コミットメッセージ・要件・設計書）から**明示された情報**を抽出する
2. 不足している情報を**不明な情報**としてリストアップする
3. 不明な情報がある場合はユーザーに確認し、回答を得るまでレビューを開始しない
4. 確認結果はレビュー結果と合わせて記録する
