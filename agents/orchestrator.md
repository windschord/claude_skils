...
1. `.orchestrating-agents/` ディレクトリを作成（存在しなければ）+ `.gitignore` に一時ファイルのパターンを追加（**環境準備の最初に実施**。`mission.md`、`task_ledger.md`、`session_state.md`、`archive/`。`workorders/` は追跡対象）
2. `orchestrating-agents/assets/templates/mission_template_ja.md` を Read で読み込み → `mission.md` を作成
3. タスクを分解し TaskCreate で各タスクを登録
4. `orchestrating-agents/assets/templates/task_ledger_template_ja.md` を Read で読み込み → `task_ledger.md` を作成
5. `orchestrating-agents/assets/templates/session_state_template_ja.md` を Read で読み込み → `session_state.md` を作成

**注意**: ステップ2, 4, 5ではテンプレートをReadツールでフルパス指定で読み込んでから使用すること。独自フォーマットでの作成は禁止。
...
**重要**: `subagent_type` は必ず `"general-purpose"` を使用する。子が孫を起動する3階層モードでは Agent tool へのアクセスが必須であり、general-purpose 型はこの要件を確実に満たす。一貫性と確実性のため、専用型ではなく general-purpose に統一する。
