...
1. `.orchestrating-agents/` ディレクトリを作成（存在しなければ）+ `.gitignore` に一時ファイルのパターンを追加（**環境準備の最初に実施**。`.orchestrating-agents/mission.md`、`.orchestrating-agents/task_ledger.md`、`.orchestrating-agents/session_state.md`、`.orchestrating-agents/archive/`。`workorders/` は追跡対象）
