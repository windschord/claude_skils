...
2. `.gitignore` に一時ファイルのパターンを追加（**最初に実施**。`mission.md`、`task_ledger.md`、`session_state.md`、`archive/`。`workorders/` は追跡対象）
3. `mission_template_ja.md` を Read で読み込み → `mission.md` を作成
4. タスクを分解し TaskCreate で各タスクを登録
5. `task_ledger_template_ja.md` を Read で読み込み → `task_ledger.md` を作成
6. `session_state_template_ja.md` を Read で読み込み → `session_state.md` を作成

**注意**: ステップ3, 5, 6ではテンプレートをReadツールで読み込んでから使用すること。独自フォーマットでの作成は禁止。
...
**重要**: `subagent_type` は必ず `"general-purpose"` を使用する。`orchestrating-agents:manager` 等は使用しない。general-purpose型でないとAgent toolにアクセスできず、子が孫を起動できない。
