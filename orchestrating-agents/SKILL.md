...
```text
1. git diff HEAD~1..HEAD -- . ':(exclude)*.test.*' ':(exclude)*_test.*' ':(exclude)*_spec.*' ':(exclude)__tests__/**' ':(exclude)test/**' ':(exclude)tests/**'
   → マージ直後に実行し、直近のマージコミットで導入されたテスト関連ファイル以外の変更を確認
