...
**重要**: 子は必ず `subagent_type: "general-purpose"` で起動すること。子が孫を起動する3階層モードでは Agent tool へのアクセスが必須であり、general-purpose 型はこの要件を確実に満たす。専用型の中にも tools に Agent を含むものがあるが、一貫性と確実性のため general-purpose に統一する。
...
```text
1. git diff HEAD~1..HEAD -- ':(exclude)*.test.*' ':(exclude)*_test.*' ':(exclude)*_spec.*'
   → 直近のマージコミットで導入された変更のうち、テストファイル以外の変更がないか確認
   （制約がある場合。HEAD~1..HEAD はマージ直後に実行し、マージで取り込んだ差分を確認する）
