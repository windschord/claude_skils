...
1. git diff HEAD~1..HEAD -- . ':(exclude)*.test.*' ':(exclude)*_test.*' ':(exclude)*_spec.*'
   → マージ直後に実行し、直近のマージコミットで導入されたテストファイル以外の変更を確認
   （ベースパス '.' を明示してから除外パターンを適用。制約違反の検出に使用）
2. 差分に想定外のプロダクションコード変更がないか確認
   → 判定基準: 子に与えた制約（例: テストファイルのみ変更可）に含まれないファイルが変更されていれば違反
3. 全体テスト・ビルドを実行して統合問題を検出
   → 判定基準: テスト全件パス + ビルドエラーゼロ。失敗があれば該当マージを git revert で取り消す
