...
2. 目標・成功基準を解釈・記録
3. 実行計画を策定
4. .gitignore に .orchestrating-agents/ を追加（未登録の場合）
```
...
```text
.orchestrating-agents/          # セッション固有の一時ファイル（.gitignore推奨）
├── mission.md                  # 親のミッション定義
├── task_ledger.md              # タスク台帳（ユーザー確認用）
...
**注意**: `.orchestrating-agents/` はセッション固有の一時データであり、`.gitignore` に追加してバージョン管理対象外とすることを推奨する。レジュームはローカルファイルシステム上のデータと `Agent tool(resume: agent_id)` で実現するため、git追跡は不要。

- **mission.md**: 親のみが更新
- **task_ledger.md**: 親のみが更新（ユーザーが常時確認可能）
