...
★ 振り返りフェーズ ★
  ...
  3. 記録保存: .orchestrating-agents/reflection.md に保存（git追跡対象）
  ↓
振り返りサマリーをユーザーに提示
```
...
1. タスクの分解は想定通りでしたか？
2. 実行の優先順位は適切でしたか？
3. エスカレーション（確認）のタイミングは適切でしたか？
4. 最終成果物は期待通りの品質でしたか？
5. 作業全体のスピードは許容範囲内でしたか？

「いいえ」の回答があった場合は、具体的な改善点をフォローアップで確認する。

### 記録の保存

- 保存先: `.orchestrating-agents/reflection.md`（git追跡対象）
- テンプレート: `orchestrating-agents/assets/templates/reflection_template_ja.md`（**Read で読み込んでから使用**）
- 複数回の振り返りはセッション日時をヘッダーにして追記
...
| 振り返り | `orchestrating-agents/assets/templates/reflection_template_ja.md` | ミッション完了後 |
