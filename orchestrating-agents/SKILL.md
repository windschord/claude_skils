...
```text
□ 1. .orchestrating-agents/ ディレクトリを作成
□ 2. .gitignore に一時ファイルパターンを追加（未登録の場合）
□ 3. mission.md を作成（テンプレート必須参照）
□ 4. タスク分解 + TaskCreate でキュー登録
□ 5. task_ledger.md を作成（テンプレート必須参照）
□ 6. session_state.md を作成（テンプレート必須参照）
```
...
**注意**: .gitignore の更新は最初に行うこと。後回しにしない。
...
```text
1. assets/templates/mission_template_ja.md を Read ツールで読み込む（必須）
2. ユーザーの原文指示を保存
3. 目標・成功基準を解釈・記録
4. 実行計画を策定
```
...
3. assets/templates/task_ledger_template_ja.md を Read ツールで読み込む（必須）
...
```text
1. assets/templates/session_state_template_ja.md を Read ツールで読み込む（必須）
...
**重要**: 子は必ず `subagent_type: "general-purpose"` で起動すること。`orchestrating-agents:manager` 等の専用型は使用しない。general-purpose 型でないと Agent tool にアクセスできず、3階層モードで孫を起動できない。

テンプレート: `assets/templates/parent_prompt_template_ja.md`（**Read ツールで読み込んでから使用**）
...
```text
1. git diff main -- ':(exclude)*.test.*' ':(exclude)*_test.*' ':(exclude)*_spec.*'
   → テストファイル以外の変更がないか確認（制約がある場合）
2. 想定外のプロダクションコード変更がないか目視レビュー
3. 全体テスト・ビルドを実行して統合問題を検出
4. 問題があれば git revert で該当マージを取り消し、子に再指示
```

**注意**: 子エージェントが制約（例: テストファイルのみ変更可）を破ってプロダクションコードを変更するケースがある。マージ後の差分チェックを怠ると、PRレビューまで検知できない。
...
| ファイル | テンプレート | タイミング |
|---------|------------|----------|
| mission.md | `assets/templates/mission_template_ja.md` | 起動時 |
| task_ledger.md | `assets/templates/task_ledger_template_ja.md` | 起動時 |
| session_state.md | `assets/templates/session_state_template_ja.md` | 起動時 |
| 親→子 指示 | `assets/templates/parent_prompt_template_ja.md` | 子セッション起動時 |
| 子→親 報告 | `assets/templates/report_template_ja.md` | 子セッション完了時 |
| workorder | `assets/templates/workorder_template_ja.md` | 子セッション内で起動時 |
| 子→孫 指示 | `assets/templates/child_prompt_template_ja.md` | 孫セッション起動時 |
