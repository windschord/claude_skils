...
**【重要】ドキュメント作成・修正後は、逆順（タスク → 設計 → 要件）でレビューを行う**

ドキュメントを作成または修正した後、以下の逆順レビューを実施してください：
...
tasks.mdの各タスクについて、design.mdとの整合性を確認：
...
**不整合の例**:
- タスクに「UserServiceを実装」とあるが、design.mdにUserServiceの定義がない
- タスクのAPI仕様がdesign.mdのAPI設計と異なる

### ステップ2: 設計 → 要件の整合性チェック

design.mdの各コンポーネント・機能について、requirements.mdとの整合性を確認：

| チェック項目 | 確認内容 |
|-------------|---------|
| 機能カバレッジ | すべての要件（REQ-XXX）に対応する設計要素があるか |
| 非機能要件対応 | NFR-XXXの要件が設計に反映されているか |
| ユーザーストーリー対応 | 各ユーザーストーリーを実現する設計になっているか |
| 過剰設計チェック | requirements.mdにない機能が設計に含まれていないか |

**不整合の例**:
- REQ-003の要件に対応するコンポーネントがdesign.mdにない
- design.mdに「管理者ダッシュボード」があるが、requirements.mdに該当要件がない
...
- [ ] すべてのタスクが設計書のコンポーネント/APIに対応している
- [ ] すべての設計要素が要件に対応している
- [ ] 要件に対応しないタスクや設計がない（過剰実装の防止）
- [ ] 設計やタスクに対応しない要件がない（実装漏れの防止）
- [ ] 不整合がある場合はユーザーに確認済み
...
**【重要】すべての設計・タスク作成において、情報を分類してから進める**

ユーザーの指示を受け取ったら、まず以下のプロセスを実行してください：

#### ステップ1: 情報の分類
ユーザーの指示から以下を明確に分けてリストアップする：

**明示された情報（Explicit）**:
- ユーザーが明確に述べた要件、仕様、制約
- 具体的な値、ファイルパス、技術名など
- 例: 「TypeScriptを使う」「PostgreSQLを使う」「/api/usersエンドポイント」

**不明な情報（Unknown）**:
- ユーザーが言及していないが、実装に必要な情報
- 「おそらく〜だろう」と推測が必要な項目
- 複数の選択肢がありユーザーの意図が不明な項目
- 例: 「認証方式は？」「エラーハンドリングの方針は？」「テストカバレッジの目標は？」

#### ステップ2: 不明情報の確認
**不明な情報が1つでもあれば、実装前に必ず質問する**

```
設計/タスク作成の前に、以下の点を確認させてください：
...
- 設計書・タスクには「明示された情報」と「確認済みの情報」のみを記載
- 推測や仮定に基づく内容は含めない

#### 「おそらく〜だろう」ルール
以下のような推測的な思考が発生した場合は、**必ず「不明」として扱う**：

- 「おそらくJWTを使うだろう」→ 不明：認証方式を確認
- 「たぶんこのパターンだろう」→ 不明：設計パターンを確認
- 「一般的には〜なので」→ 不明：このプロジェクトでの方針を確認
- 「〜と思われる」→ 不明：具体的な仕様を確認
...
**【重要】サブエージェントが自律的に作業できるよう、情報を完全に記載する**

タスクを実行するサブエージェントが質問なしに作業を完了できるよう、以下の情報を必ず含めてください：

#### 必須情報チェックリスト

各タスクに以下の情報が含まれているか確認してください：
