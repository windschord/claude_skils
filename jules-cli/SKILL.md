...
**完了日時**: 2024-01-15 14:00（= PRマージ日時。PRベースの場合はマージ完了をもって完了とする）
**作業時間**: 3時間30分
```
...
**各ステータスの詳細**:
- `TODO`: 未着手
- `IN_PROGRESS`: Julesが作業中（jules依頼後）
- `REVIEW`: JulesがPRを作成済み、レビュー待ち
- `DONE`: PRがレビュー承認され、開発ブランチにマージ済み
...
エージェントチームとJulesを組み合わせることで、最大効率のタスク実行を実現します。チームリーダーがJulesの進捗監視・PRレビュー・タスク管理を担当し、複数のJulesインスタンスとローカルAgentが並列でタスクを処理します。
...
チームリーダー（オーケストレーター）
├── Jules依頼管理:
│   ├── Jules-1: TASK-001（PR #41）→ 非同期で実行
│   ├── Jules-2: TASK-002（PR #42）→ 非同期で実行
│   └── Jules-3: TASK-003（PR #43）→ 非同期で実行
├── ローカルAgent（Agent Teams/Task tool）:
│   ├── Agent-1: TASK-004（ローカル環境依存タスク）
│   └── Agent-2: TASK-005（対話的判断が必要なタスク）
└── 管理業務:
    ├── jules listで定期的にJulesの進捗確認
    ├── PRレビュー・マージ管理
...
```text
1. タスク一覧を分析し、依存関係グラフを構築
2. 並列実行可能なグループを特定
3. 各タスクをJules向き/ローカル向きに分類
4. ユーザーに割り当て計画を提示し承認を得る
5. Jules向けタスク: 複数のjulesコマンドで同時依頼
6. ローカル向けタスク: Agent Teams/Task toolで並列実行
7. リーダーはjules listで定期的にJulesの進捗を確認
8. ローカルAgentの完了報告を待機
9. JulesのPR作成を確認し、レビュー・マージ
10. 全タスク完了後にdocs/sdd/tasks/とTodoWriteを一括更新
11. 逆順レビューを実施
```

### 進捗管理の一元化

リーダーがJulesとローカルAgentの進捗を一元管理します:

```text
進捗確認コマンド:
  Jules:        jules list
  ローカルAgent: チームメンバーからの完了メッセージ

ステータス更新:
  Jules PR作成   → TASK-XXX.md: REVIEW, index.md更新, TodoWrite同期
  Jules PRマージ → TASK-XXX.md: DONE, index.md更新, TodoWrite同期
  Agent完了      → TASK-XXX.md: DONE, index.md更新, TodoWrite同期
```
